# adapters/biconomy_adapter.py — FULL UPDATED WITH cancel_all_orders
import os
import time
import hashlib
import requests
...
class BiconomyAdapter(BatchCancelMixin, BaseAdapter):
    ...
    def cancel_orders_by_ids(self, order_ids: List[str]):
        ...
        self._cancel_in_batches(
            order_ids,
            "/api/v1/private/trade/cancel_batch",
            payload_func,
            batch_size=10,
        )
    ...
    def cancel_all_orders(self):
        """
        Cancels ALL open Biconomy orders using doc-verified batch cancel.
        """
        if self.dry_run:
